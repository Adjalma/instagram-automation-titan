...
var SDKServer = class {
  ...
  async authenticateRequest(req) {
    ...
    try {
      ...
      console.warn("[Auth] getUserByOpenId falhou:", err?.message);
      user = void 0;
    }
    if (!user) {
    ...
    void upsertUser({ openId: user.openId, lastSignedIn: /* @__PURE__ */ new Date() }).catch((err) => {
      console.warn("[Auth] lastSignedIn update skipped:", err?.message);
    });
    return user;
  }
  async loginWithPassword(email, password) {
    ...
    try {
      ...
      console.warn("[Auth] getUserByEmail falhou:", err?.message);
      user = void 0;
    }
    if (!user || !user.passwordHash) return null;
    const valid = await comparePassword(password, user.passwordHash);
    if (!valid) return null;
    void upsertUser({ openId: user.openId, lastSignedIn: /* @__PURE__ */ new Date() }).catch(() => {
    });
    return user;
  }
